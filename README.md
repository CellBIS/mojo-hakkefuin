...
- Built-in SQLite (default), MariaDB/MySQL, and PostgreSQL backends with automatic migration creation (`migrations/mhf_<backend>.sql`).
...
plugin 'Hakkefuin' => {
  ...
  dsn             => 'postgresql://user:pass@localhost/mhf', # needed for pg/mariadb
};

post '/login' => sub {
  my $c   = shift;
  my $id  = $c->param('user');
  my $res = $c->auth_signin($id);          # stores cookie+csrf in DB
  return $c->render(status => $res->{code}, json => $res);
};

under sub {
  my $c    = shift;
  my $auth = $c->auth_has_auth;             # checks cookie+csrf and stashes ids
  return $c->render(status => 423, json => $auth) if $auth->{result} == 2;
  return $c->render(status => 401, text  => 'Unauthorized') unless $auth->{result} == 1;
  $c->auth_csrf;                            # ensure token is in session/header
  return 1;
};

get '/me' => sub {
  my $c = shift;
  return $c->render(json => { user => $c->stash('auth.identify') });
};
...
- Create the migration file under `dir` when missing and ensure the auth table exists.
...
- SQLite stores data in `dir/mhf_sqlite.db` and creates `dir/mhf_sqlite.sql` with schema + indexes.
- MariaDB/MySQL and PostgreSQL create `mhf_mariadb.sql` or `mhf_pg.sql` under `dir` and ensure indexes for identify, cookie, and expire_date.
